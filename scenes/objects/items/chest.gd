...
@export var pop_left = false
@export var unlock_audio: AudioStreamWAV
@export var chest_opened_audio: AudioStreamOggVorbis
...
@onready var pop_out_direction: Node2D = $PopOutDirection
...
func on_health_changed():
	if not is_opened:
		if detect_enemy_area_2d.get_overlapping_bodies().is_empty():
			...
			%AudioStreamPlayer2D.stream = unlock_audio
